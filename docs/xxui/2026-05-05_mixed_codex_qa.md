...
一、框架命名与 API 风格
  ...
  1. 最终包？
A:  xxui
  1. 组件 API 是否统一小写：app.button()、app.card()，还是保留类名风格：app.Button()、app.Card()？
A:  统一小写
  1. ui.use(Panel()) 和 ui.App(render=PanelRender()) 二选一，最终采用哪个？
A:
import xxui as ui
...
app = ui.App(
  ...
  1. 是否允许模块级快捷调用 ui.text()，还是强制所有 UI 创建都从 app.text() 进入？
A: 必须从一个容器进行，app.xxx 避免全局变量导致测试、代码难以多实例难写的问题
  1. parent.context.button() 这种写法是否保留，还是只保留 app.button() + current context？
...
A. 参考61 ,marimo默认是可以嵌入panel组件的，所以即便xxui不开发marimo实现，理论上也可以把xxui 的panel挂在marimo里
...
A: 是的,封装panel/marimo inputtext
  ...
  app=PanelApp()
  name:xxui.panel.TextInput = app.textInput("Name")
  name.signal
...
A: signal本身是独立的，应该直接可以xxui.signal.signal()类构造器实例化,但是因为在ui中,我们的app限定只对scope signal rerun，而app.signal()是挂载创建scope signal的方法，所以这个场景必须使用。
